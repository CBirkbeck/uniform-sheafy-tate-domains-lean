...
`RationalLocData.IsRational.span_eq_top` (Wedhorn 7.30(1)) needs `[IsTateRing 𝓐]`, and
the `HasLocLiftPowerBounded`/`restrictionMap` vocabulary needs `[IsHuberRing E]` at each
vertex. Over the abstract base these do *not* require a uniformizer:
`NontriviallyNormedField K` already provides a scaling constant `c` with `0 < ‖c‖ < 1`
(the inverse of a norm-`>1` element), and the constants of each jet ring scale the norm
exactly. So — unlike the ϖ-parametric pseudouniformizer package `piA …`/`isTateRing_JetA …`
of `Over/JetRings.lean`, which the graph–Koszul engine consumes — the bare topological
classes are honest instances, and with them the faithful pair-free loc-lift discharger
applies to all four rings unconditionally. -/
...
noncomputable def constB (c : K) : JetB K := TrivSqZeroExt.inl (constHomPS K c)
...
noncomputable def constD (c : K) : JetD K := TrivSqZeroExt.inl (RestrictedLaurent.C c)
